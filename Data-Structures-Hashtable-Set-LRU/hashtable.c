...
hashtable_t *hashtable_new(const int num_slots) {
    ...
    // alocate memory, cos dynamic sets
...
bool hashtable_insert(hashtable_t *ht, const char *key, void *item) {
    ...
    if (!set_insert(ht->bins[index], new, item)) {
        free(new);
        return false; // Memory allocation failure
    }
...
void *hashtable_find(hashtable_t *ht, const char *key) {
    ...
    // return null 
    if (ht == NULL || key == NULL || set_find(ht->bins[index], key) == NULL) {
        return NULL;
    }
...
void hashtable_print(hashtable_t *ht, FILE *fp, void (*itemprint)(FILE *fp, const char *key, void *item)) {
    ...
    for (int x = 0; x < ht -> num_slots; x++) {
        //print using set file
        set_print( ht->bins[x], fp, itemprint);
    }
}
...
void hashtable_iterate(hashtable_t *ht, void *arg, void (*itemfunc)(void *arg, const char *key, void *item)) {
    ...
    for (int a = 0; a < ht->num_slots; a++) {
        set_iterate( ht->bins[a], arg, itemfunc);
    }
}
...
void hashtable_delete(hashtable_t *ht, void (*itemdelete)(void *item)) {
    ...
    for (int b = 0; b < ht->num_slots; b++) {
        // use set to delet 
        set_delete(ht->bins[b], itemdelete);
    }
